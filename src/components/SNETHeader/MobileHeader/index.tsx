  ...
  return (
    ...
    >
      ...
      <nav className={classes.mobileNavigation}>
        <ul className={`${isLoggedIn ? null : classes.hideNav} ${color === "white" && classes.whiteHeader}`}>
          {mobileNavLinks.map((tab) => (
            <NavItem key={tab.title} title={tab.title} link={tab.link} isActive={tab.isActive} />
          ))}
          {mobileDropDown &&
            mobileDropDown.map((dropdown) => (
              <div key={dropdown.label} className={classes.subMenues}>
                ...
                {dropdown.list.map((item: MenuItemProps) => (
                  <NavItem key={item.label} title={item.label} link={item.link} />
                ))}
              </div>
            ))}
        </ul>
